...
class ZoomifyBase:
  ...
  #   row = 0
  #   ul_y, lr_y = (0,0)
  #   root, ext = os.path.splitext(os.path.basename(self._v_imageFilename)) 
  #   if not root:
  #     root = self._v_imageFilename
  #   ext = '.jpg'

  #   # Define the directory to save the images and ensure it exists
  #   save_directory = os.path.join(tempfile.gettempdir(), "zoomify_temp")
  ...
  #     # Save the image in the previously ensured directory
  #     imageRow.save(os.path.join(save_directory, saveFilename), 'JPEG', quality=100)
  #     image = None
  #     imageRow = None
  #     if os.path.exists(os.path.join(save_directory, saveFilename)):
  ...
  def processImage(self):
    ...
    for tier in range(len(self._v_scaleInfo)):
        
        row = 0
        tierWidth, tierHeight = self._v_scaleInfo[tier]
        tierWidth, tierHeight = int(tierWidth), int(tierHeight)
        # print(f"Tier width: {tierWidth}, Tier height: {tierHeight}") 

        while row * self.tileSize < tierHeight:
            ul_y = row * self.tileSize
            if (ul_y + self.tileSize) < tierHeight:
                lr_y = ul_y + self.tileSize
            else:
                lr_y = tierHeight
            ...
            scaledImage = image.resize((tierWidth, tierHeight), PIL.Image.ANTIALIAS)
            imageRow = scaledImage.crop([0, ul_y, tierWidth, lr_y])

            root, ext = os.path.splitext(os.path.basename(self._v_imageFilename))
            imageRowPath = os.path.join(save_directory, f"{root}_{tier}-{row}{ext}")

            if imageRow.mode != 'RGB':
                imageRow = imageRow.convert('RGB')

            imageRow.save(imageRowPath, 'JPEG', quality=100)
            # print(f"Saved {imageRowPath}")

            if os.path.exists(imageRowPath):
                self.processRowImage(tier=tier, row=row)
  ...
  #   print(f"Processing row image for tier {tier}, row {row}")
  ...
  #     imageRow = PIL.Image.new('RGB', (int(tierWidth), int(self.tileSize)))
  ...
  #       tile = imageRow.crop([ul_x, ul_y, lr_x, lr_y])
  #       # self.saveTile(imageRow.crop([ul_x, ul_y, lr_x, lr_y]), tier, column, row)
  #       if tile.size[0] > 0 and tile.size[1] > 0:
  #         self.saveTile(tile, tier, column, row)
  #         print(f"ROWIMAGE_Tile saved at tier {tier}, row {row}, column {column}")
  #       else:
  #         print(f"ROWIMAGE_Skipped saving an empty tile at tier {tier}, row {row}, column {column}")
  ...
  #       if imageHeight > 1:
  #         tempImage = imageRow.resize((int(imageWidth/2), int(imageHeight/2)), PIL.Image.ANTIALIAS)
  ...
  def processRowImage(self, tier=0, row=0):
    ...
    imageRowPath = os.path.join(save_directory, f"{root}_{tier}-{row}{ext}")
    if not os.path.exists(imageRowPath):
        # print(f"Image row file does not exist: {imageRowPath}")
        return
    
    imageRow = PIL.Image.open(imageRowPath)
    imageRowWidth, imageRowHeight = imageRow.size
    # print(f"Row width: {imageRowWidth}, Row height: {imageRowHeight}") 
    if imageRowWidth == 0 or imageRowHeight == 0:
        # print(f"Empty image row: {imageRowPath}")
        return

    column = 0
    while column * self.tileSize < imageRowWidth:
        ul_x = column * self.tileSize
        lr_x = min(ul_x + self.tileSize, imageRowWidth)
        ul_y = 0
        lr_y = min(self.tileSize, imageRowHeight)

        
        tile = imageRow.crop((ul_x, ul_y, lr_x, lr_y))
        if tile.size[0] > 0 and tile.size[1] > 0:
            self.saveTile(tile, tier, column, row)
            # print(f"ROWIMAGE_Tile saved at tier {tier}, row {row}, column {column} with dimensions {tile.size}")
            self.numberOfTiles += 1
        # else:
            # print(f"ROWIMAGE_Skipped saving an empty tile at tier {tier}, row {row}, column {column} with dimensions {tile.size}")
