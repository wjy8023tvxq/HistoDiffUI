...
class ZoomifyFileProcessor(ZoomifyBase):
  ...
  def saveTile(self, image, scaleNumber, column, row):
    ...
    w, h = image.size
    if w != 0 and h != 0:
        file_path = self.getFileReference(scaleNumber, column, row)
        image.save(file_path, 'JPEG', quality=self.qualitySetting)
        print(f"Tile saved: {file_path}")
    else:
        print(f"Skipped saving an empty tile at scale {scaleNumber}, column {column}, row {row}.")
